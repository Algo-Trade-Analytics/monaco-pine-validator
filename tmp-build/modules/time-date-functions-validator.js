...
export class TimeDateFunctionsValidator {
    constructor() {
        this.name = 'TimeDateFunctionsValidator';
        this.priority = 75; // Medium priority - time functions are important but not critical
        this.errors = [];
        this.warnings = [];
        this.info = [];
        this.astContext = null;
        this.hasTimezoneReference = false;
        this.timeComparisonEmissions = new Set();
        this.timeArithmeticEmissions = new Set();
    ...
    collectTimeDataFromAst(program) {
        const loopStack = [];
        visit(program, {
            ForStatement: {
                enter: () => {
            ...
            MemberExpression: {
                enter: (path) => {
                    this.processAstMember(path);
                },
            },
            BinaryExpression: {
                enter: (path) => {
                    this.processAstBinary(path.node);
                },
            },
            StringLiteral: {
                enter: (path) => {
                    this.processAstString(path.node);
                },
            },
        });
    }
    ...
    processAstMember(path) {
        ...
        if (objectName === 'timezone') {
            this.addWarning(line, column, `Identifier '${qualifiedName}' is not a valid Pine Script timezone constant. Provide an explicit string (e.g., "UTC") or use syminfo.timezone.`, 'PSV6-TIMEZONE-UNKNOWN');
            return;
        }
        if (objectName === 'session' && !VALID_SESSION_MEMBERS.has(qualifiedName)) {
            this.addWarning(line, column, `Unknown session constant: ${qualifiedName}`, 'PSV6-SESSION-UNKNOWN');
        }
    }
    processAstBinary(node) {
    ...
    validateTimeFunction(functionName, args, lineNum, column) {
        switch (functionName) {
            case 'time_close':
    ...
    validateTimeClose(args, lineNum, column) {
        ...
        if (!this.isValidTimeframeParameter(timeframeParam)) {
            this.addWarning(lineNum, column, `First parameter for time_close should be a timeframe (string or variable), got ${timeframeParam}`, 'PSV6-TIME-CLOSE-TIMEFRAME');
        }
        let sessionParam = null;
        let timezoneParam = null;
        let barsBackParam = null;
        if (args.length >= 2) {
        ...
        if (sessionParam) {
            if (!this.isValidSessionParameter(sessionParam)) {
                this.addWarning(lineNum, column, `Invalid session parameter for time_close: ${sessionParam}. Use session.regular, session.extended, or a valid session string.`, 'PSV6-TIME-CLOSE-SESSION');
            }
        }
        if (timezoneParam) {
        ...
        if (args.length > 4) {
            this.addWarning(lineNum, column, 'time_close supports up to four parameters: timeframe, session, timezone, bars_back. Extra arguments will be ignored by Pine Script.', 'PSV6-TIME-CLOSE-PARAMS-EXTRA');
        }
        this.addInfo(lineNum, column, 'time_close calculates the bar close time for a given timeframe and session. Ensure timezone/bars_back are set when needed.', 'PSV6-TIME-CLOSE-INFO');
    }
    validateTimeTradingday(args, lineNum, column) {
        if (args.length < 1) {
        ...
        if (!this.isValidTimeParameter(timeParam)) {
            this.addWarning(lineNum, column, `Time parameter should be a time value or time variable: ${timeParam}`, 'PSV6-TIME-TRADINGDAY-TIME');
        }
    ...
    validateTimestamp(args, lineNum, column) {
        if (args.length < 6) {
        ...
        if (!isNaN(yearValue) && (yearValue < 1970 || yearValue > 2100)) {
            this.addWarning(lineNum, column, `Year value ${yearValue} is outside typical range (1970-2100)`, 'PSV6-TIMESTAMP-YEAR-RANGE');
        }
        // Validate month parameter
        const monthParam = args[1].trim();
        const monthValue = parseInt(monthParam);
        if (!isNaN(monthValue) && (monthValue < 1 || monthValue > 12)) {
            this.addError(lineNum, column, `Month value must be between 1 and 12, got ${monthValue}`, 'PSV6-TIMESTAMP-MONTH-RANGE');
        }
        // Validate day parameter
        const dayParam = args[2].trim();
        const dayValue = parseInt(dayParam);
        if (!isNaN(dayValue) && (dayValue < 1 || dayValue > 31)) {
            this.addError(lineNum, column, `Day value must be between 1 and 31, got ${dayValue}`, 'PSV6-TIMESTAMP-DAY-RANGE');
        }
        ...
        if (!isNaN(hourValue) && (hourValue < 0 || hourValue > 23)) {
            this.addError(lineNum, column, `Hour value must be between 0 and 23, got ${hourValue}`, 'PSV6-TIMESTAMP-HOUR-RANGE');
        }
        ...
        if (!isNaN(minuteValue) && (minuteValue < 0 || minuteValue > 59)) {
            this.addError(lineNum, column, `Minute value must be between 0 and 59, got ${minuteValue}`, 'PSV6-TIMESTAMP-MINUTE-RANGE');
        }
        ...
        if (!isNaN(secondValue) && (secondValue < 0 || secondValue > 59)) {
            this.addError(lineNum, column, `Second value must be between 0 and 59, got ${secondValue}`, 'PSV6-TIMESTAMP-SECOND-RANGE');
        }
        // Validate optional timezone parameter
        if (args.length >= 7) {
    ...
    validateTimenow(args, lineNum, column) {
        if (args.length > 0) {
    ...
    validateTimezoneParameter(timezoneParam, lineNum, column) {
        ...
        if (this.isStringLiteral(trimmed)) {
            ...
            if (timezoneValue.length === 0) {
                ...
                this.addWarning(lineNum, column, `String '${timezoneValue}' looks like a session name. Use session.* constants for sessions.`, 'PSV6-TIMEZONE-SESSION-MISUSE');
            }
            else {
                this.addInfo(lineNum, column, `Custom timezone specified: ${timezoneValue}. Ensure it's a valid timezone identifier.`, 'PSV6-TIMEZONE-CUSTOM');
            }
            return;
        }
        ...
        if (trimmed.startsWith('timezone.')) {
            this.addWarning(lineNum, column, `Identifier '${trimmed}' is not a valid Pine Script timezone constant. Provide a string like "UTC" or use syminfo.timezone.`, 'PSV6-TIMEZONE-INVALID');
            this.addEnumMismatchWarning(lineNum, column, 'Comparing enum values from different types');
            return;
        }
        if (!this.isVariableReference(trimmed)) {
            this.addWarning(lineNum, column, `Timezone argument '${trimmed}' should be a string literal, syminfo.timezone, or a variable containing a timezone string.`, 'PSV6-TIMEZONE-UNKNOWN');
            this.addEnumMismatchWarning(lineNum, column, 'Comparing enum values from different types');
        }
    }
    isValidSessionParameter(sessionParam) {
    ...
    isValidTimeframeParameter(timeframeParam) {
        ...
        if (trimmed === 'timeframe.period')
            return true;
        if (trimmed.startsWith('timeframe.'))
            return true;
        if (trimmed.startsWith('input.timeframe'))
            return true;
        if (this.isVariableReference(trimmed))
            return true;
        return false;
    }
    isTimezoneArgument(value) {
        ...
        if (trimmed.startsWith('timezone.'))
            return true;
        if (this.isVariableReference(trimmed) && trimmed.toLowerCase().includes('timezone'))
            return true;
        return false;
    }
    validateBarsBackParameter(param, lineNum, column) {
    ...
    validateTimePerformance() {
        // Check for too many time function calls
        if (this.timeFunctionCalls.length > 10) {
            this.addWarning(1, 1, `High number of time function calls (${this.timeFunctionCalls.length}). Consider caching results.`, 'PSV6-TIME-PERF-MANY-CALLS');
        }
        // Check for complex time expressions
        if (this.complexTimeExpressions > 5) {
            this.addWarning(1, 1, `Many complex time expressions (${this.complexTimeExpressions}). Consider simplifying for better performance.`, 'PSV6-TIME-PERF-COMPLEX');
        }
        // Check for time functions in loops
        this.checkTimeFunctionsInLoops();
    }
    validateTimeBestPractices() {
        ...
        if (hasTimeClose && hasTimeTradingday) {
            this.addInfo(1, 1, 'Good time handling pattern - using both session close and trading day functions.', 'PSV6-TIME-BEST-PRACTICE');
        }
        if (hasTimestamp) {
            this.addInfo(1, 1, 'Using timestamp function for specific time calculations - ensure timezone handling is correct.', 'PSV6-TIMESTAMP-BEST-PRACTICE');
        }
        ...
        if ((hasTimeClose || hasTimeTradingday || hasTimestamp) && !hasTimezone) {
            this.addInfo(1, 1, 'Consider specifying timezone for time calculations to ensure consistent behavior across markets.', 'PSV6-TIMEZONE-SUGGESTION');
        }
    }
    checkTimeFunctionsInLoops() {
        // Check if time functions are called inside loops (performance concern)
        for (const timeCall of this.timeFunctionCalls) {
            if (timeCall.inLoop) {
                this.addWarning(timeCall.line, timeCall.column, `Time function ${timeCall.functionName} inside loop may impact performance`, 'PSV6-TIME-PERF-LOOP');
            }
        }
    }
    emitTimeComparison(node, identifier, compared) {
        ...
        this.addWarning(loc.line, loc.column, `Direct comparison of ${identifier} with ${comparedText} may be fragile. Consider using time ranges or functions.`, 'PSV6-TIME-DIRECT-COMPARE');
    }
    emitTimeArithmetic(node, identifier) {
        const loc = node.loc?.start;
        if (!loc) {
            return;
        }
        ...
        this.addInfo(loc.line, loc.column, `Time arithmetic detected with ${identifier}. Ensure proper time unit handling.`, 'PSV6-TIME-ARITHMETIC-INFO');
    }
    getTimeVariableIdentifier(expression) {
