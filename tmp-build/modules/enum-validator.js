...
export class EnumValidator {
    constructor() {
        this.name = 'EnumValidator';
        this.errors = [];
        this.warnings = [];
        this.info = [];
        this.astContext = null;
        this.astEnumDeclarations = new Map();
        this.astEnumVariables = new Map();
        this.astFunctionParams = new Map();
    }
    getDependencies() {
        return ['UDTValidator'];
    }
    ...
    validateWithAst(program) {
        visit(program, {
            EnumDeclaration: {
                enter: (path) => {
                    this.processEnumDeclaration(path.node);
                },
            },
            FunctionDeclaration: {
                enter: (path) => {
                    this.recordFunctionDeclaration(path.node);
                },
            },
        });
        visit(program, {
            VariableDeclaration: {
                enter: (path) => {
                    this.processVariableDeclaration(path.node);
                },
            },
            MemberExpression: {
                enter: (path) => {
                    this.processMemberExpression(path.node);
                },
            },
            BinaryExpression: {
                enter: (path) => {
                    this.processBinaryExpression(path.node);
                },
            },
            CallExpression: {
                enter: (path) => {
                    this.processCallExpression(path.node);
                },
            },
            SwitchStatement: {
                enter: (path) => {
                    this.processSwitchStatement(path.node);
                },
            },
        });
    }
    processEnumDeclaration(node) {
        ...
        const seen = new Set();
        const info = { node, members: new Map() };
    ...
    processBinaryExpression(node) {
        ...
        const { line, column } = node.loc.start;
        if (identifier) {
            this.addWarning(line, column, `Comparing enum variable of type ${variableType} with ${enumType} enum value`, 'PSV6-ENUM-COMPARISON-TYPE-MISMATCH');
        }
        else {
            this.addWarning(line, column, 'Comparing enum values from different types', 'PSV6-ENUM-COMPARISON-TYPE-MISMATCH');
        }
    ...
    processCallExpression(node) {
        ...
        node.args.forEach((arg, index) => {
            ...
            this.addError(line, column, `Function parameter type mismatch: expected ${expected}, got ${reference.enumName}`, 'PSV6-ENUM-FUNCTION-TYPE-MISMATCH');
        });
    }
    processSwitchStatement(node) {
        ...
        node.cases.forEach((caseNode) => {
            ...
            if (!enumInfo.members.has(reference.memberName)) {
                const { line, column } = reference.node.property.loc.start;
                this.addError(line, column, `Undefined enum value in switch case: ${reference.enumName}.${reference.memberName}`, 'PSV6-ENUM-SWITCH-CASE-TYPE-MISMATCH');
                return;
            }
            this.recordTypeUsage(`${reference.enumName}.${reference.memberName}`, reference.node.property);
            if (discriminantType && discriminantType !== reference.enumName) {
                const { line, column } = caseNode.loc.start;
                this.addError(line, column, `Switch case enum type mismatch: expected ${discriminantType}, got ${reference.enumName}`, 'PSV6-ENUM-SWITCH-CASE-TYPE-MISMATCH');
            }
        });
    }
    resolveExpressionEnumType(expression) {
        if (!expression) {
            return null;
        }
    ...
    getEnumMemberReference(expression) {
    ...
    isKnownNamespace(name) {
        return KNOWN_NAMESPACES.has(name);
    }
    setTypeMapEntry(name, node, overrides = {}) {
