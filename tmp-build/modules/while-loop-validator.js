...
export class WhileLoopValidator {
    constructor() {
        this.name = 'WhileLoopValidator';
        this.errors = [];
        this.warnings = [];
        this.info = [];
    }
    ...
    validateWhileLoopsAst(program) {
        ...
        visit(program, {
            WhileStatement: {
                enter: (path) => {
                    ...
                    if (complexity > 2) {
                        const { line, column } = statement.loc.start;
                        this.addWarning(line, column, 'Complex operation inside while loop may impact performance', 'PSV6-WHILE-COMPLEX-OPERATION');
                    }
                },
                exit: () => {
                    loopStack.pop();
        ...
        if (maxDepth && maxDepth.depth > 3) {
            this.addWarning(maxDepth.line, maxDepth.column, `While loop nesting depth is ${maxDepth.depth}. Consider refactoring.`, 'PSV6-WHILE-DEEP-NESTING');
        }
    }
    evaluateWhileCondition(statement) {
        const { line, column } = statement.loc.start;
        const { test } = statement;
        switch (test.kind) {
            case 'BooleanLiteral':
                if (test.value) {
                    this.addError(line, column, 'While loop with condition "true" will run indefinitely', 'PSV6-WHILE-INFINITE-LOOP');
                }
                else {
                    this.addWarning(line, column, 'While loop with condition "false" will never execute', 'PSV6-WHILE-NEVER-EXECUTES');
                }
                break;
            case 'NumberLiteral':
                this.addWarning(line, column, 'While loop with numeric condition may not behave as expected', 'PSV6-WHILE-NUMERIC-CONDITION');
                break;
            case 'StringLiteral':
                this.addWarning(line, column, 'While loop with string condition may not behave as expected', 'PSV6-WHILE-STRING-CONDITION');
                break;
            default:
                break;
        }
        ...
        if (analysis.logicalOperators >= 3) {
            this.addWarning(line, column, 'While loop condition is complex. Consider simplifying.', 'PSV6-WHILE-COMPLEX-CONDITION');
        }
        if (analysis.comparisonOperators > 2) {
            this.addInfo(line, column, 'Consider breaking complex condition into multiple variables', 'PSV6-WHILE-CONDITION-SIMPLIFICATION');
        }
        if (analysis.hasEquality && !analysis.hasInequality) {
            this.addInfo(line, column, 'Consider using != instead of == for while loop conditions', 'PSV6-WHILE-CONDITION-BEST-PRACTICE');
        }
        if (analysis.firstComparisonIdentifier) {
            this.addInfo(line, column, 'Ensure loop variable is updated inside the loop to prevent infinite loops', 'PSV6-WHILE-VARIABLE-UPDATE-REMINDER');
            const identifier = analysis.firstComparisonIdentifier;
            if (identifier.name.length < 3) {
                this.addInfo(line, column, 'Consider using more descriptive variable names in while loop conditions', 'PSV6-WHILE-VARIABLE-NAMING');
            }
        }
    }
    analyseConditionExpression(expression) {
        const state = {
            logicalOperators: 0,
            comparisonOperators: 0,
            hasEquality: false,
            hasInequality: false,
            firstComparisonIdentifier: null,
        };
    ...
    extractComparisonIdentifier(expression) {
    ...
    analyseWhileBody(body) {
        let complexity = 0;
        const breakLines = new Set();
        const conditionalBreakLines = new Set();
            ...
            IfStatement: {
                enter: (path) => {
                    ...
                    if (this.containsBreakOrReturn(node) && !conditionalBreakLines.has(node.loc.start.line)) {
                        ...
                        this.addInfo(node.loc.start.line, node.loc.start.column, 'Conditional break/return in while loop. Ensure all code paths lead to termination.', 'PSV6-WHILE-CONDITIONAL-BREAK');
                    }
                },
            },
            ForStatement: {
            ...
            CallExpression: {
                enter: (path) => {
                    ...
                    if (qualified && this.isExpensiveOperation(qualified)) {
                        this.addWarning(node.loc.start.line, node.loc.start.column, `Expensive operation "${qualified}" inside while loop may impact performance`, 'PSV6-WHILE-EXPENSIVE-OPERATION');
                    }
            ...
            AssignmentStatement: {
                enter: (path) => {
                    ...
                    if (target && this.isLoopCounterName(target) && !updateLines.has(node.loc.start.line)) {
                        ...
                        this.addInfo(node.loc.start.line, node.loc.start.column, `Loop variable "${target}" updated. Good practice for preventing infinite loops.`, 'PSV6-WHILE-VARIABLE-UPDATE-GOOD');
                    }
                },
            },
            BreakStatement: {
                enter: (path) => {
                    ...
                    if (!breakLines.has(node.loc.start.line)) {
                        ...
                        this.addInfo(node.loc.start.line, node.loc.start.column, 'Break/return statement found in while loop. Ensure proper loop termination.', 'PSV6-WHILE-BREAK-CONDITION');
                    }
                },
            },
            ReturnStatement: {
    ...
    containsBreakOrReturn(statement) {
        switch (statement.kind) {
            ...
            case 'IfStatement':
                return (this.containsBreakOrReturn(statement.consequent) ||
    ...
    getAssignmentTarget(expression) {
