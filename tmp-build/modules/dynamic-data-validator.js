...
export class DynamicDataValidator {
    constructor() {
        this.name = 'DynamicDataValidator';
        this.errors = [];
        this.warnings = [];
        this.info = [];
    ...
    collectRequestCallsAst(program) {
        const loopStack = [];
    ...
    validateRequestCalls() {
        for (const call of this.requestCalls) {
            if (!VALID_REQUEST_FUNCTIONS.has(call.functionName)) {
                this.addError(call.line, call.column, `Unknown request function: ${call.name}`, 'PSV6-REQUEST-UNKNOWN');
                continue;
            }
            this.handleRequestCall(call);
        }
    }
    handleRequestCall(call) {
    ...
    validateRequestSecurityCall(call) {
        if (call.arguments.length < 3) {
            this.reportRequestParamError(call.line, call.column, 'request.security requires at least 3 parameters (symbol, timeframe, expression)', 'PSV6-REQUEST-SECURITY-PARAMS');
        }
        ...
        if (symbolArg && !this.argumentIsStringLiteral(symbolArg)) {
            ...
            this.addWarning(position.line, position.column, 'Dynamic symbol parameter detected in request.security', 'PSV6-REQUEST-DYNAMIC-SYMBOL');
            this.addInfo(position.line, position.column, 'Series symbol parameters are valid in Pine v6', 'PSV6-REQUEST-DYNAMIC-V6');
        }
        ...
        if (timeframeArg &&
            ...
            this.addWarning(position.line, position.column, 'Dynamic timeframe parameter detected in request.security', 'PSV6-REQUEST-DYNAMIC-TIMEFRAME');
            this.addInfo(position.line, position.column, 'Series timeframe parameters are valid in Pine v6', 'PSV6-REQUEST-DYNAMIC-V6');
            this.addError(position.line, position.column, 'Timeframe parameter must be a string literal when calling request.security.', 'PSV6-FUNCTION-PARAM-TYPE');
            this.addError(position.line, position.column, 'Enum mismatch: request.security timeframe should use a literal timeframe string.', 'PSV6-ENUM-TYPE-MISMATCH');
        }
    }
    validateSecurityParameters(call) {
        const symbolArg = this.getNamedArgument(call, 'symbol') ?? this.getPositionalArgument(call, 0);
        if (symbolArg && this.argumentIsStringLiteral(symbolArg)) {
    ...
    validateRequestSecurityLowerTfCall(call) {
        if (call.arguments.length < 3) {
            this.reportRequestParamError(call.line, call.column, 'request.security_lower_tf requires at least 3 parameters (symbol, timeframe, expression)', 'PSV6-REQUEST-SECURITY-LOWER-TF-PARAMS');
            return;
        }
    }
    validateRequestCurrencyRateCall(call) {
        if (call.arguments.length < 2) {
            this.reportRequestParamError(call.line, call.column, 'request.currency_rate requires at least 2 parameters (from, to)', 'PSV6-REQUEST-CURRENCY-RATE-PARAMS');
            return;
        }
        ...
        if (fromArg) {
            ...
            if (this.argumentIsStringLiteral(fromArg)) {
                ...
                if (!/^[A-Za-z]{3}$/.test(value)) {
                    this.addWarning(position.line, position.column, `Unexpected currency code "${value}". Use ISO 4217 codes (e.g., "USD").`, 'PSV6-REQUEST-CURRENCY-CODE');
                }
            }
            else {
                this.addError(position.line, position.column, 'Currency source must be a string literal (e.g., "USD").', 'PSV6-FUNCTION-PARAM-TYPE');
                this.addError(position.line, position.column, 'Undefined currency enum type for request.currency_rate source parameter.', 'PSV6-ENUM-UNDEFINED-TYPE');
            }
        }
        ...
        if (toArg) {
            ...
            if (this.argumentIsStringLiteral(toArg)) {
                ...
                this.addError(position.line, position.column, 'Currency target must be a string literal (e.g., "JPY").', 'PSV6-FUNCTION-PARAM-TYPE');
                this.addError(position.line, position.column, 'Undefined currency enum type for request.currency_rate target parameter.', 'PSV6-ENUM-UNDEFINED-TYPE');
            }
        }
        ...
        if (ignoreArg && this.argumentIsStringLiteral(ignoreArg)) {
            ...
            if (literal !== 'true' && literal !== 'false') {
                ...
                this.addWarning(position.line, position.column, `ignore_invalid_currency should be boolean. Received "${literal}"`, 'PSV6-REQUEST-CURRENCY-IGNORE');
            }
        }
    }
    validateRequestSeedCall(call) {
        if (call.arguments.length < 3) {
            this.reportRequestParamError(call.line, call.column, 'request.seed requires at least 3 parameters (source, symbol, expression)', 'PSV6-REQUEST-SEED-PARAMS');
            return;
        }
        ...
        if (sourceArg && !this.argumentIsStringLiteral(sourceArg)) {
            ...
            this.addWarning(position.line, position.column, 'Seed source should be a string literal referencing the repository name.', 'PSV6-REQUEST-SEED-SOURCE');
        }
        ...
        if (symbolArg) {
            ...
            if (!this.argumentIsStringLiteral(symbolArg)) {
                this.addError(position.line, position.column, 'Seed symbol must be a string literal reference to the CSV file.', 'PSV6-REQUEST-SEED-PARAMS');
                this.addWarning(position.line, position.column, 'Seed file name should be a string literal without the .csv extension.', 'PSV6-REQUEST-SEED-SYMBOL');
                this.addEnumMismatchWarning(position.line, position.column, 'Enum mismatch: request.seed symbol should reference a literal file name');
            }
            else {
                const text = symbolArg.value.trim();
                if (/\.csv['"]?$/.test(text)) {
                    this.addWarning(position.line, position.column, 'Seed symbol should omit the .csv suffix; provide the base file name only.', 'PSV6-REQUEST-SEED-EXT');
                    this.addEnumMismatchWarning(position.line, position.column, 'Enum mismatch: request.seed symbol should omit the .csv suffix');
                }
            }
        }
        ...
        if (this.isMissingSeedExpression(expressionArg)) {
            ...
            this.addError(position.line, position.column, 'Seed requests require an expression or tuple to evaluate.', 'PSV6-REQUEST-SEED-EXPRESSION');
        }
        ...
        if (calcBarsArg) {
            ...
            if (this.argumentIsStringLiteral(calcBarsArg)) {
                this.addWarning(position.line, position.column, 'calc_bars_count should be an integer, not a string literal.', 'PSV6-REQUEST-SEED-CALC-BARS');
            }
            else if (calcBarsArg.value.trim() && !this.argumentIsIntegerLiteral(calcBarsArg)) {
                this.addInfo(position.line, position.column, 'Dynamic calc_bars_count detected. Ensure the value stays within plan limits.', 'PSV6-REQUEST-SEED-CALC-BARS-DYNAMIC');
            }
        }
        this.checkAdvancedRequestPerformance(call, 'seed');
    }
    validateRequestQuandlCall(call) {
        if (call.arguments.length < 2) {
            this.reportRequestParamError(call.line, call.column, 'request.quandl requires at least 2 parameters (database, code)', 'PSV6-REQUEST-QUANDL-PARAMS');
        }
        ...
        if (databaseArg && this.argumentIsStringLiteral(databaseArg)) {
            ...
            if (!value.includes('/')) {
                ...
                this.addWarning(position.line, position.column, `Quandl database should be in format "DATABASE/CODE": ${value}`, 'PSV6-REQUEST-QUANDL-FORMAT');
            }
        }
    }
    validateRequestDividendsCall(call) {
        if (call.arguments.length < 2) {
            this.reportRequestParamError(call.line, call.column, 'request.dividends requires at least 2 parameters (ticker, field)', 'PSV6-REQUEST-DIVIDENDS-PARAMS');
            return;
        }
        ...
        if (fieldArg && this.argumentIsStringLiteral(fieldArg)) {
            ...
            if (!valid.includes(value)) {
                ...
                this.addWarning(position.line, position.column, `Unknown dividend field: ${value}. Valid fields: ${valid.join(', ')}`, 'PSV6-REQUEST-DIVIDENDS-FIELD');
                this.addEnumMismatchWarning(position.line, position.column, `Enum mismatch: request.dividends field should use ${valid.join(' or ')}`);
            }
        }
    ...
    validateRequestSplitsCall(call) {
        if (call.arguments.length < 2) {
            this.reportRequestParamError(call.line, call.column, 'request.splits requires at least 2 parameters (ticker, field)', 'PSV6-REQUEST-SPLITS-PARAMS');
            return;
        }
        const fieldArg = this.getNamedArgument(call, 'field') ?? this.getPositionalArgument(call, 1);
        if (fieldArg && this.argumentIsStringLiteral(fieldArg)) {
            ...
            if (!valid.includes(value)) {
                const position = this.getArgumentPosition(call, fieldArg);
                this.addWarning(position.line, position.column, `Unknown split field: ${value}. Valid fields: ${valid.join(', ')}`, 'PSV6-REQUEST-SPLITS-FIELD');
                this.addEnumMismatchWarning(position.line, position.column, `Enum mismatch: request.splits field should use ${valid.join(' or ')}`);
            }
        }
        const gapsArg = this.getNamedArgument(call, 'gaps') ?? this.getPositionalArgument(call, 2);
    ...
    validateRequestEarningsCall(call) {
        if (call.arguments.length < 2) {
            this.reportRequestParamError(call.line, call.column, 'request.earnings requires at least 2 parameters (ticker, field)', 'PSV6-REQUEST-EARNINGS-PARAMS');
            return;
        }
        const fieldArg = this.getNamedArgument(call, 'field') ?? this.getPositionalArgument(call, 1);
        if (fieldArg) {
            ...
            if (fieldName && !valid.includes(fieldName)) {
                const position = this.getArgumentPosition(call, fieldArg);
                this.addWarning(position.line, position.column, `Unknown earnings field: ${fieldName}. Valid fields: ${valid.join(', ')}`, 'PSV6-REQUEST-EARNINGS-FIELD');
                this.addEnumMismatchWarning(position.line, position.column, `Enum mismatch: request.earnings field should use ${valid.join(', ')}`);
            }
        }
        const gapsArg = this.getNamedArgument(call, 'gaps') ?? this.getPositionalArgument(call, 2);
    ...
    validateRequestEconomicCall(call) {
        if (call.arguments.length < 2) {
            this.reportRequestParamError(call.line, call.column, 'request.economic requires at least 2 parameters (country_code, field)', 'PSV6-REQUEST-ECONOMIC-PARAMS');
        }
        ...
        if (countryArg) {
            ...
            if (this.argumentIsStringLiteral(countryArg)) {
                ...
                if (!validCountryCodes.includes(country.toUpperCase())) {
                    this.addWarning(position.line, position.column, `Unknown country code: ${country}. Common codes: ${validCountryCodes.join(', ')}`, 'PSV6-REQUEST-ECONOMIC-COUNTRY');
                    this.addEnumMismatchWarning(position.line, position.column, 'Enum mismatch: request.economic country_code must use a supported ISO code');
                }
            }
            else {
                this.addError(position.line, position.column, 'Country code must be provided as a string literal (e.g., "US").', 'PSV6-FUNCTION-PARAM-TYPE');
                this.addError(position.line, position.column, 'Undefined enum type for request.economic country_code parameter.', 'PSV6-ENUM-UNDEFINED-TYPE');
            }
        }
        const fieldArg = this.getNamedArgument(call, 'field') ?? this.getPositionalArgument(call, 1);
        if (fieldArg && this.argumentIsStringLiteral(fieldArg)) {
            ...
            if (!validEconomicFields.includes(fieldValue)) {
                const position = this.getArgumentPosition(call, fieldArg);
                this.addWarning(position.line, position.column, `Unknown economic field: ${fieldValue}. Valid fields: ${validEconomicFields.join(', ')}`, 'PSV6-REQUEST-ECONOMIC-FIELD');
                this.addEnumMismatchWarning(position.line, position.column, `Enum mismatch: request.economic field should use ${validEconomicFields.join(', ')}`);
            }
        }
        const gapsArg = this.getNamedArgument(call, 'gaps') ?? this.getPositionalArgument(call, 2);
    ...
    validateRequestFinancialCall(call) {
        if (call.arguments.length < 3) {
            const fieldArg = this.getNamedArgument(call, 'financial_id') ?? this.getPositionalArgument(call, 1);
            if (fieldArg && !this.argumentIsStringLiteral(fieldArg)) {
                const position = this.getArgumentPosition(call, fieldArg);
                this.addWarning(position.line, position.column, 'Second parameter should be a financial field string (e.g., "TOTAL_REVENUE")', 'PSV6-REQUEST-FINANCIAL-FIELD');
            }
            this.reportRequestParamError(call.line, call.column, 'request.financial requires at least 3 parameters (symbol, financial_id, period)', 'PSV6-REQUEST-FINANCIAL-PARAMS');
            return;
        }
        const symbolArg = this.getNamedArgument(call, 'symbol') ?? this.getPositionalArgument(call, 0);
        if (symbolArg && !this.argumentIsStringLiteral(symbolArg)) {
            const position = this.getArgumentPosition(call, symbolArg);
            this.addWarning(position.line, position.column, 'First parameter should be a symbol string literal', 'PSV6-REQUEST-SYMBOL-FORMAT');
            this.addError(position.line, position.column, 'Financial symbol must be a string literal when calling request.financial.', 'PSV6-FUNCTION-PARAM-TYPE');
            this.addError(position.line, position.column, 'Undefined enum type for request.financial symbol parameter.', 'PSV6-ENUM-UNDEFINED-TYPE');
        }
        ...
        if (financialIdArg && this.argumentIsStringLiteral(financialIdArg)) {
            ...
            if (!validIds.includes(value)) {
                ...
                this.addWarning(position.line, position.column, `Unknown financial ID: ${value}. Valid IDs include: ${validIds.slice(0, 5).join(', ')}, etc.`, 'PSV6-REQUEST-FINANCIAL-ID');
                this.addEnumMismatchWarning(position.line, position.column, 'Enum mismatch: request.financial financial_id should reference a supported identifier');
            }
        }
        else if (financialIdArg) {
            const position = this.getArgumentPosition(call, financialIdArg);
            this.addWarning(position.line, position.column, 'Second parameter should be a financial field string (e.g., "TOTAL_REVENUE")', 'PSV6-REQUEST-FINANCIAL-FIELD');
            this.addEnumMismatchWarning(position.line, position.column, 'Enum mismatch: request.financial financial_id must be a recognized enum literal');
        }
        ...
        if (periodArg && this.argumentIsStringLiteral(periodArg)) {
            ...
            if (!validPeriods.includes(period)) {
                ...
                this.addWarning(position.line, position.column, `Unknown financial period: ${period}. Valid periods: ${validPeriods.join(', ')}`, 'PSV6-REQUEST-FINANCIAL-PERIOD');
                this.addEnumMismatchWarning(position.line, position.column, 'Enum mismatch: request.financial period should be FY, FQ, TTM, or FH');
            }
        }
    ...
    validateGapsParameter(arg, call, functionType) {
        if (!arg) {
            return;
        }
        ...
        if (this.argumentIsStringLiteral(arg)) {
            ...
            if (!validValues.includes(value)) {
                this.addWarning(position.line, position.column, `Invalid gaps parameter for request.${functionType}: ${value}. Valid values: ${validValues.join(', ')}`, 'PSV6-REQUEST-GAPS-INVALID');
                this.addEnumMismatchWarning(position.line, position.column, `Enum mismatch: request.${functionType} gaps should use ${validValues.join(', ')}`);
            }
            return;
        }
        if (trimmed.startsWith('barmerge.gaps_')) {
            if (!validValues.includes(trimmed)) {
                this.addWarning(position.line, position.column, `Invalid barmerge gaps constant: ${trimmed}. Use barmerge.gaps_on or barmerge.gaps_off`, 'PSV6-REQUEST-GAPS-BARMERGE');
                this.addEnumMismatchWarning(position.line, position.column, `Enum mismatch: request.${functionType} gaps should use barmerge.gaps_on or barmerge.gaps_off`);
            }
            return;
        }
        if (!this.argumentIsBooleanLiteral(arg)) {
            this.addInfo(position.line, position.column, `Dynamic gaps parameter detected in request.${functionType}. Ensure it resolves to a valid gaps value.`, 'PSV6-REQUEST-GAPS-DYNAMIC');
        }
    }
    validateGapsNamedArguments(call, functionType) {
    ...
    validateLookaheadParameter(arg, call, functionType) {
        if (!arg) {
            return;
        }
        const trimmed = arg.value.trim();
        ...
        if (this.argumentIsStringLiteral(arg)) {
            ...
            if (!validValues.includes(value)) {
                this.addWarning(position.line, position.column, `Invalid lookahead parameter for request.${functionType}: ${value}. Valid values: ${validValues.join(', ')}`, 'PSV6-REQUEST-LOOKAHEAD-INVALID');
                this.addEnumMismatchWarning(position.line, position.column, `Enum mismatch: request.${functionType} lookahead should use ${validValues.join(', ')}`);
            }
            return;
        }
        if (trimmed.startsWith('barmerge.lookahead_')) {
            if (!validValues.includes(trimmed)) {
                this.addWarning(position.line, position.column, `Unknown lookahead constant: ${trimmed}. Use barmerge.lookahead_on or barmerge.lookahead_off.`, 'PSV6-REQUEST-LOOKAHEAD-INVALID');
                this.addEnumMismatchWarning(position.line, position.column, `Enum mismatch: request.${functionType} lookahead should use barmerge.lookahead_on or barmerge.lookahead_off`);
            }
            return;
        }
        if (!this.argumentIsBooleanLiteral(arg)) {
            this.addInfo(position.line, position.column, `Dynamic lookahead parameter detected in request.${functionType}. Ensure it evaluates to barmerge.lookahead_* or a boolean.`, 'PSV6-REQUEST-LOOKAHEAD-DYNAMIC');
        }
    }
    checkAdvancedRequestPerformance(call, functionType) {
        if (EXPENSIVE_REQUEST_FUNCTIONS.has(functionType)) {
            this.addInfo(call.line, call.column, `request.${functionType} may have slower response times. Consider caching results.`, 'PSV6-REQUEST-PERFORMANCE-EXPENSIVE');
        }
        if (!this.advancedPerformanceWarned) {
            ...
            if (advancedCount > 5) {
                this.addWarning(call.line, call.column, `Multiple advanced request functions detected (${advancedCount}). Consider performance impact.`, 'PSV6-REQUEST-PERFORMANCE-MULTIPLE');
                this.advancedPerformanceWarned = true;
            }
        }
    }
    emitEnumComparisonWarning(call) {
        const enumPrefixes = [
            'barmerge.',
            'dividends.',
            'splits.',
        ...
        if (hasEnumLiteral || hasComplexExpression) {
            this.addEnumMismatchWarning(call.line, call.column, 'Comparing enum values from different types');
        }
    }
    validateRequestPerformance() {
        ...
        for (const call of this.requestCalls) {
            ...
            if (EXPENSIVE_REQUEST_FUNCTIONS.has(call.functionName)) {
                this.addInfo(call.line, call.column, 'Advanced request function may be expensive; consider minimizing calls', 'PSV6-REQUEST-PERFORMANCE-EXPENSIVE');
            }
    ...
    validateDynamicContexts() {
        for (const call of this.requestCalls) {
            if (!call.hasDynamicExpression) {
                continue;
            }
            if (call.inLoop) {
                this.addWarning(call.line, call.column, 'Dynamic request inside loop may cause heavy performance usage', 'PSV6-REQUEST-DYNAMIC-LOOP');
            ...
            if (call.inConditional) {
                this.addWarning(call.line, call.column, 'Dynamic request inside conditional block may cause inconsistent performance', 'PSV6-REQUEST-DYNAMIC-CONDITIONAL');
    ...
    createArgumentInfoFromAst(argument) {
    ...
    computeDynamicExpression(functionName, args) {
    ...
    getExpressionArgument(functionName, args) {
    ...
    expressionIsDynamic(node) {
        switch (node.kind) {
            ...
            case 'ArrayLiteral': {
                ...
                return arrayLiteral.elements.some((element) => element ? this.expressionIsDynamic(element) : false);
            }
            default:
                return false;
        }
    }
    isDynamicExpressionText(text) {
    ...
    getNamedArgument(call, name) {
        return call.arguments.find((arg) => arg.name === name);
    }
    getPositionalArgument(call, index) {
        let count = 0;
        for (const arg of call.arguments) {
            if (arg.name) {
                continue;
    ...
    countPositionalArguments(call) {
        return call.arguments.filter((arg) => !arg.name).length;
    }
    getArgumentPosition(call, arg) {
        if (arg?.node) {
            return {
    ...
    isMissingSeedExpression(arg) {
        ...
        if (arg.node) {
            ...
            if (arg.node.kind === 'Identifier' && arg.node.name === 'na') {
                return true;
            }
        }
        return false;
    }
    argumentIsStringLiteral(arg) {
        if (!arg) {
            return false;
        }
    ...
    validateTickerSymbolFormat(symbol, line, column) {
        ...
        if (trimmedSymbol.includes(' ')) {
            this.addWarning(line, column, `Symbol "${trimmedSymbol}" contains spaces. Use underscores or proper format.`, 'PSV6-REQUEST-SYMBOL-FORMAT');
        }
        const validFormats = [
            /^[A-Z]{1,5}$/,
            /^[A-Z]{1,5}:[A-Z]{1,5}$/,
        ...
        if (!isValidFormat) {
            if (trimmedSymbol.length > 20) {
                ...
                this.addWarning(line, column, `Symbol "${trimmedSymbol}" contains invalid characters. Use alphanumeric, colon, dot, underscore, or dash only.`, 'PSV6-REQUEST-SYMBOL-CHARS');
            }
            else {
                this.addInfo(line, column, `Symbol "${trimmedSymbol}" format not recognized. Verify it's a valid ticker symbol.`, 'PSV6-REQUEST-SYMBOL-UNKNOWN');
            }
        }
        if (trimmedSymbol.includes(':')) {
