...
export class LinefillValidator {
    constructor() {
        this.name = 'LinefillValidator';
        this.priority = 85; // High priority - linefills are important v6 drawing features
        this.errors = [];
        this.warnings = [];
        this.info = [];
        ...
        this.linefillOperations = new Map();
        this.linefillCount = 0;
        this.linefillIdentifiers = new Set();
    ...
    collectLinefillDataFromAst(program) {
        const loopStack = [];
        visit(program, {
            VariableDeclaration: {
                enter: (path) => {
                    this.registerAstLinefillDeclaration(path.node);
                },
            },
            AssignmentStatement: {
                enter: (path) => {
                    this.registerAstLinefillAssignment(path.node);
                },
            },
            ForStatement: {
                enter: () => {
            ...
            CallExpression: {
                enter: (path) => {
                    this.processAstCall(path.node, loopStack.length > 0);
                },
            },
        });
    }
    processAstCall(call, inLoop) {
    ...
    registerAstLinefillDeclaration(declaration) {
    ...
    registerAstLinefillAssignment(assignment) {
    ...
    registerLinefillIdentifier(identifier) {
    ...
    recordLinefillCall(functionName, args, lineNum, column, inLoop) {
    ...
    validateLinefillFunction(functionName, args, lineNum, column) {
    ...
    validateLinefillNew(args, lineNum, column) {
        if (args.length < 2) {
    ...
    validateLinefillSetColor(args, lineNum, column) {
        if (args.length !== 2) {
    ...
    validateLinefillDelete(args, lineNum, column) {
        if (args.length !== 1) {
    ...
    validateLinefillGetLine1(args, lineNum, column) {
        if (args.length !== 1) {
    ...
    validateLinefillGetLine2(args, lineNum, column) {
        if (args.length !== 1) {
    ...
    validateLinefillPerformance() {
        // Check for too many linefill objects
        if (this.linefillCount > 10) {
            this.addWarning(0, 0, `Too many linefill objects (${this.linefillCount}). Consider optimizing for better performance.`, 'PSV6-LINEFILL-PERF-MANY');
        }
    }
    validateLinefillBestPractices() {
        ...
        for (const [key, count] of callCounts) {
            if (count >= 3 && !this.hasCacheSuggestion) {
                ...
                this.addInfo(1, 1, `Multiple similar linefill operations detected. Consider caching results.`, 'PSV6-LINEFILL-CACHE-SUGGESTION');
        ...
        if (this.linefillCount >= 5 && !this.hasCleanupSuggestion) {
            this.addInfo(1, 1, 'Consider using linefill.delete() to clean up unused linefills and improve performance.', 'PSV6-LINEFILL-CLEANUP-SUGGESTION');
    ...
    getLinefillFunctionCalls() {
        return [...this.linefillFunctionCalls];
    }
    getLinefillOperations() {
    ...
    isLinefillNamespace(expression) {
        return expression.kind === 'Identifier' && expression.name === 'linefill';
    }
    isLinefillNewCall(call) {
        if (call.callee.kind !== 'MemberExpression') {
            return false;
        }
        const member = call.callee;
