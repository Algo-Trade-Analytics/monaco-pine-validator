...
export class FunctionTypesValidator {
    constructor() {
        this.name = 'FunctionTypesValidator';
        this.priority = 93; // High priority - runs after function declarations and built-in validation
        this.errors = [];
        this.warnings = [];
        this.info = [];
    ...
    getFunctionReturnType(funcName) {
        ...
        if (funcName.startsWith('array.')) {
            ...
            if (member.startsWith('new'))
                return 'array';
            if (['copy', 'slice', 'concat', 'from', 'from_example', 'range', 'sort_indices'].includes(member))
                return 'array';
            if (['size', 'indexof', 'lastindexof', 'binary_search', 'binary_search_leftmost', 'binary_search_rightmost'].includes(member))
                return 'int';
            if (member === 'get')
                return 'unknown';
            if (['push', 'pop', 'set', 'clear', 'reverse', 'sort', 'remove', 'insert', 'unshift', 'shift', 'fill', 'standardize'].includes(member))
                return 'void';
            if (['some', 'every'].includes(member))
                return 'bool';
            ...
            FunctionDeclaration: {
                enter: (path) => {
                    ...
                    if (returnTypes.length > 1) {
                        this.addError(location.loc.start.line, location.loc.start.column, `Function '${name}' has inconsistent return types: ${returnTypes.join(', ')}`, 'PSV6-FUNCTION-RETURN-TYPE');
                    }
                },
            },
        });
        ...
        visit(program, {
            FunctionDeclaration: {
                enter: (path) => {
                    ...
                    if (complexity > 10) {
                        this.addWarning(location.loc.start.line, location.loc.start.column, `Function '${name}' has high complexity (${complexity}). Consider breaking it into smaller functions.`, 'PSV6-FUNCTION-COMPLEXITY');
                    }
                    const length = this.calculateFunctionLengthAst(fn);
                    if (length > 50) {
                        this.addWarning(location.loc.start.line, location.loc.start.column, `Function '${name}' is very long (${length} lines). Consider breaking it into smaller functions.`, 'PSV6-FUNCTION-LENGTH');
                    }
                },
            },
        });
    }
    collectReturnTypesAst(fn) {
        if (!this.astContext) {
            return [];
        }
        const types = new Set();
    ...
    validateReturnTypeUsage(funcName, call) {
        ...
        if (returnType === 'series' && this.isBooleanFunction(funcName)) {
            ...
            if (this.isArithmeticContext(beforeCall, afterCall)) {
                this.addError(call.line, call.column, `Boolean function '${funcName}' cannot be used in arithmetic operations`, 'PSV6-FUNCTION-RETURN-TYPE');
            }
        }
        // Check for string operations with non-string return types
        if (returnType !== 'string' && this.isStringContext(beforeCall, afterCall)) {
            this.addError(call.line, call.column, `Function '${funcName}' returns ${returnType}, cannot be used in string operations`, 'PSV6-FUNCTION-RETURN-TYPE');
        }
    }
    /**
     ...
     */
    isArithmeticContext(before, after) {
        const arithmeticOps = ['+', '-', '*', '/', '%', '^'];
