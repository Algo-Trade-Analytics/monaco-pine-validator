...
export class FunctionDeclarationsValidator {
    constructor() {
        this.name = 'FunctionDeclarationsValidator';
        this.priority = 95; // High priority - function declarations are fundamental
        this.errors = [];
        this.warnings = [];
        this.info = [];
        // Function tracking
    ...
    collectStaticFunctionDeclarations(lines) {
        const START_STATIC = new RegExp(`^\\s*(?:export\\s+)?static\\s+(${IDENT.source})\\s+(${IDENT.source})\\s*\\(`);
        const FULL_STATIC = new RegExp(`^\\s*(?:export\\s+)?static\\s+(${IDENT.source})\\s+(${IDENT.source})\\s*\\(([\\s\\S]*?)\\)\\s*=>`, 'm');
        let buffer = '';
        let startIndex = -1;
        let linesSeen = 0;
    ...
    extractNamesFromRawParams(params) {
        return params
            .map((param) => {
