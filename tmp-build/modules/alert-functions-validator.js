...
export class AlertFunctionsValidator {
    constructor() {
        this.name = 'AlertFunctionsValidator';
        this.priority = 75; // Medium priority - alert functions are important for notifications
        this.errors = [];
        this.warnings = [];
        this.info = [];
        ...
        this.alertFrequencyUsage = new Map();
        this.recordedFrequencyKeys = new Set();
    }
    getDependencies() {
    ...
    collectAlertDataFromAst(program) {
        visit(program, {
            CallExpression: {
                enter: (path) => this.processAstAlertCall(path),
            },
            MemberExpression: {
                enter: (path) => this.processAstMemberExpression(path),
            },
            Identifier: {
    ...
    processAstAlertCall(path) {
        ...
        const alertCall = {
            ...
            inLoop: !!findAncestor(path, (ancestor) => ancestor.node.kind === 'ForStatement' ||
    ...
    validateAstAlertCall(call, alertCall) {
        ...
        if (frequencyPath && VALID_ALERT_FREQUENCIES.has(frequencyPath)) {
            ...
            this.errors.push({
                ...
                message: `Invalid alert frequency '${frequencyPath}'. Use alert.freq_all, alert.freq_once_per_bar, or alert.freq_once_per_bar_close`,
                line,
                column,
                severity: 'error',
            });
    ...
    validateAstAlertConditionCall(call) {
    ...
    resolveConditionType(expression) {
        if (!expression) {
            return null;
        }
    ...
    resolveExpressionPath(expression) {
        if (expression.kind === 'Identifier') {
    ...
    getAlertFrequencyPath(expression) {
        if (expression.kind === 'MemberExpression') {
            const member = expression;
            if (member.computed) {
                return null;
    ...
    recordAlertFrequencyUsage(frequency, line, column) {
