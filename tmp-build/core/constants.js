...
export const QUALIFIED_FN_RE = new RegExp(`^\\s*(?:export\\s+)?(${IDENT.source}(?:\\.${IDENT.source})*)\\s*\\(([^)]*)\\)\\s*=>`);
export const METHOD_DECL_RE = new RegExp(`^\\s*method\\s+(${IDENT.source})\\s*\\(([^)]*)\\)\\s*=>`);
// Variable declarations
export const VAR_DECL_RE = new RegExp(`^\\s*(?:(?:var|varip|const)\\s+)?` +
...
export const BUILTIN_FUNCTIONS_V6_RULES = {
