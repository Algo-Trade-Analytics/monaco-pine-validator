...
import { As, ColonEqual, Comma, Enum, Equal, FatArrow, LParen, Colon, Less, Greater, Identifier as IdentifierToken, Import, Indicator, Library, Newline, StringLiteral as StringToken, RParen, Strategy, Type, } from '../tokens';
import { buildTypeReferenceFromTokens, createBlockStatementNode, createFunctionDeclarationNode, createIdentifierFromTokens, createIdentifierNode, createImplicitReturnStatementNode, createImportDeclarationNode, createParameterNode, createPlaceholderExpression, createScriptDeclarationNode, createTypeDeclarationNode, createTypeFieldNode, createVariableDeclarationNode, createEnumDeclarationNode, createEnumMemberNode, tokenIndent, } from '../node-builders';
import { attachLoopResultBinding } from '../helpers';
import { isDeclarationKeywordToken, isExportKeywordToken, isFunctionModifierToken, splitDeclarationTokens, splitFunctionHeadTokens, toDeclarationKind, } from '../parser-utils';
const EOF_TOKEN = EOF;
export function createParameterRule(parser) {
...
export function createParameterListRule(parser) {
...
export function createFunctionDeclarationRule(parser) {
    return parser.createRule('functionDeclaration', (providedExport) => {
        let startToken = providedExport;
        let exportToken = providedExport;
        ...
        return createFunctionDeclarationNode(identifier, params, body, Boolean(exportToken), typeAnnotation, modifiers, functionStartToken);
    });
}
export function createScriptDeclarationRule(parser) {
...
export function createImportDeclarationRule(parser) {
...
export function createEnumMemberRule(parser) {
    return parser.createRule('enumMember', (_parentIndent) => {
        const identifierToken = parser.consumeToken(IdentifierToken);
        const identifier = createIdentifierNode(identifierToken);
        let value = null;
...
export function createEnumDeclarationRule(parser) {
    return parser.createRule('enumDeclaration', (providedExport) => {
        let exportToken = providedExport;
        if (!exportToken && isExportKeywordToken(parser.lookAhead(1))) {
            exportToken = parser.consumeToken(IdentifierToken);
        }
...
export function createTypeFieldRule(parser) {
...
export function createTypeDeclarationRule(parser) {
    return parser.createRule('typeDeclaration', (providedExport) => {
        let exportToken = providedExport;
        if (!exportToken && isExportKeywordToken(parser.lookAhead(1))) {
            exportToken = parser.consumeToken(IdentifierToken);
        }
...
export function createVariableDeclarationRule(parser) {
    return parser.createRule('variableDeclaration', () => {
        let declarationKind = 'simple';
        ...
        const identifierToken = parser.consumeToken(IdentifierToken);
        const identifier = createIdentifierNode(identifierToken);
        const debugMode = process.env.DEBUG_PARSER === '1' && !parser.RECORDING_PHASE;
        if (colonIndex >= 0) {
        ...
        const variableDeclaration = createVariableDeclarationNode(declarationKind, identifier, identifierToken, typeAnnotation, initializer, operatorToken ? (operatorToken.image === ':=' ? ':=' : '=') : null, startToken);
        if (debugMode) {
