...
import { And, Colon, Comma, Dot, Else, Equal, EqualEqual, False, For, Greater, GreaterEqual, Increment, Decrement, Identifier as IdentifierToken, If, LBracket, Less, LessEqual, Minus, NaToken, Newline, InvalidLogicalAnd, InvalidLogicalOr, StrictEqual, StrictNotEqual, BitwiseOr, BitwiseAnd, BitwiseXor, Bang, Not, NotEqual, NullishCoalescing, NumberLiteral as NumberToken, ColorLiteral as ColorToken, Or, Percent, Plus, Question, RBracket, Slash, Star, StringLiteral as StringToken, Switch, True, While, LParen, FatArrow, RParen, } from '../tokens';
import { createArgumentNode, createArrowFunctionExpressionNode, createBinaryExpressionNode, createBlockStatementNode, createBooleanNode, createCallExpressionNode, createConditionalExpressionNode, createIdentifierNode, createIfExpressionNode, createImplicitReturnStatementNode, createIndexExpressionNode, createArrayLiteralNode, createMatrixLiteralNode, createMemberExpressionNode, createNullNode, createNumberNode, createPlaceholderExpression, createStringNode, createColorLiteralNode, createTupleExpressionNode, createUnaryExpressionNode, buildTypeReferenceFromTokens, tokenIndent, } from '../node-builders';
...
function isArrowFunctionStart(parser) {
...
export function createArrowFunctionExpressionRule(parser) {
...
export function createExpressionRule(parser) {
    return parser.createRule('expression', () => parser.invokeSubrule(parser.conditionalExpression));
}
export function createConditionalExpressionRule(parser) {
    return parser.createRule('conditionalExpression', () => {
        ...
        if (parser.lookAhead(1).tokenType === Question) {
            ...
            return createConditionalExpressionNode(test, consequent, alternate, questionToken, colonToken, endToken);
        }
        return test;
    });
}
export function createNullishCoalescingExpressionRule(parser) {
...
export function createLogicalOrExpressionRule(parser) {
...
export function createLogicalAndExpressionRule(parser) {
...
export function createEqualityExpressionRule(parser) {
...
export function createRelationalExpressionRule(parser) {
...
export function createAdditiveExpressionRule(parser) {
...
export function createMultiplicativeExpressionRule(parser) {
...
export function createUnaryExpressionRule(parser) {
...
export function createCallTypeReferenceRule(parser) {
    return parser.createRule('callTypeReference', () => {
        const tokens = [];
...
export function createCallExpressionRule(parser) {
...
export function createMemberExpressionRule(parser) {
...
export function createArgumentListRule(parser) {
    return parser.createRule('argumentList', () => {
        const args = [];
        while (parser.lookAhead(1).tokenType === Newline) {
            parser.consumeToken(Newline);
        }
...
export function createArgumentRule(parser) {
...
export function createBracketExpressionRule(parser) {
    return parser.createRule('bracketExpression', (mode = 'expression') => {
        const lBracket = parser.consumeToken(LBracket);
        const elements = [];
        ...
        if (!hasNull) {
            const rowCandidates = elements.filter((element) => element?.kind === 'TupleExpression' || element?.kind === 'ArrayLiteral');
            if (rowCandidates.length === elements.length &&
                rowCandidates.every((row) => row.elements.every((child) => child !== null))) {
                const rows = rowCandidates.map((row) => row.elements.map((child) => child ?? createPlaceholderExpression()));
...
export function createPrimaryExpressionRule(parser) {
    return parser.createRule('primaryExpression', () => {
...
export function createIdentifierExpressionRule(parser) {
    return parser.createRule('identifierExpression', () => {
        const first = parser.consumeToken(IdentifierToken);
        let expression = createIdentifierNode(first);
        parser.repeatMany(() => {
            parser.consumeToken(Dot);
...
export function createIfExpressionRule(parser) {
    return parser.createRule('ifExpression', (baseIndentOverride) => {
...
export function createSwitchExpressionRule(parser) {
