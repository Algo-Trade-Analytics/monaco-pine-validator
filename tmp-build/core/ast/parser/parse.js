...
function tokenToSyntaxError(token, message, source, filename = '<input>') {
...
function buildProgramNode(source, directives, body) {
...
export function parseWithChevrotain(source, options = {}) {
    ...
    for (const error of lexResult.errors) {
        ...
        const text = getLine(source, line);
        syntaxErrors.push(new SyntaxError(error.message, {
            filename,
            lineno: line,
            offset: column,
            text,
