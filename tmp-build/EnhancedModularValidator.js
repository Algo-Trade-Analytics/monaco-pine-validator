...
export class EnhancedModularValidator extends BaseValidator {
    ...
    validate(codeOrContext, config) {
        if (typeof codeOrContext === 'string') {
            ...
            const source = joinLines(normalized.rawLines) ??
    ...
    getHoverInfo(position) {
        ...
        if (position.line < 1 || position.line > this.context.lines.length)
            return null;
...
export function createEnhancedModularValidator(config) {
    return new EnhancedModularValidator(config);
}
// Convenience function for validation
export function validatePineScriptV6Enhanced(code, config) {
