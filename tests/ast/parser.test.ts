...
describe('Chevrotain-based Pine parser', () => {
  it('parses version directives, declarations, and assignments', () => {
    ...
    if (declaration.kind === 'ScriptDeclaration') {
      expect(declaration.scriptType).toBe('indicator');
      expect(declaration.call.args[0]?.value.kind).toBe('StringLiteral');
    }
  });

  it('distinguishes script declarations for strategies and libraries', () => {
    const sources = [
      { code: `//@version=6\nstrategy("Strat")`, expected: 'strategy' },
      { code: `//@version=6\nlibrary("Lib")`, expected: 'library' },
    ] as const;

    for (const { code, expected } of sources) {
      const { ast, diagnostics } = service.parse(code);
      expect(diagnostics.syntaxErrors).toEqual([]);
      const [script] = ast?.body ?? [];
      expect(script?.kind).toBe('ScriptDeclaration');
      if (script?.kind === 'ScriptDeclaration') {
        expect(script.scriptType).toBe(expected);
        expect(script.call.callee.name).toBe(expected);
      }
    }
  });
