...
plot(z)`;
      ...
      // Should only have the syntax error, no cascading type errors
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0].code).toBe('PSV6-SYNTAX-EMPTY-PARAM');
      
      // Verify no cascading function/type errors (the syntax error code itself may contain these words)
      const hasCascadingErrors = result.errors.some(e => 
        e.code === 'PSV6-FUNCTION-PARAM-TYPE' || e.code === 'PSV6-TERNARY-TYPE'
      );
      expect(hasCascadingErrors).toBe(false);
    });
  });
});
