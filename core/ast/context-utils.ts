...
function ensureAstService(config: ValidatorConfig): ChevrotainAstService {
  const astConfig: AstConfig = config.ast ?? (config.ast = { mode: 'primary', service: undefined });
...
export function isAstValidationContext(context: ValidationContext): context is AstValidationContext {
  return 'ast' in context;
}

export function ensureAstContext(
