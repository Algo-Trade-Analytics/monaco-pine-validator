...
const SCRIPT_DECLARATION_IDENTIFIERS = new Set<ScriptKind>(['indicator', 'strategy', 'library']);
...
class PineCstParser extends EmbeddedActionsParser {
    ...
    return this.OR<StatementNode>([
      {
        GATE: () =>
          ...
          SCRIPT_DECLARATION_IDENTIFIERS.has(this.LA(1).image as ScriptKind) &&
  ...
  private scriptDeclaration = this.RULE<ScriptDeclarationNode>('scriptDeclaration', () => {
    const start = this.startToken();
    const scriptName = start.image as string;
    const call = this.SUBRULE(this.callExpression);
    const endToken = this.lastToken ?? start;
    const { loc, range } = this.buildLocation(start, endToken);
    const scriptType = SCRIPT_DECLARATION_IDENTIFIERS.has(scriptName as ScriptKind)
      ? (scriptName as ScriptKind)
      : 'indicator';
    return {
